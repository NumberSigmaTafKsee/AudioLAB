...
using namespace Filters::IIR::RBJFilters;

struct LV2RBJFilter : public DSP
{
	RBJBiquadFilter* filter;
	RBJBiquadFilter* stereo[2];
	DspFloatType fs2;
	DspFloatType kc=0,vel=1;
	
	LV2RBJFilter(float sr) : DSP(sr)
	{
		fs2 = sr/2.0;
	...
	void setFilterType(RBJBiquadFilter::RBJType type)
	{
		filter->setType(type);		
	...
	void Tick(LV2Plugin * m, uint32_t from, uint32_t to )
	{
		...
		LOOP(i,from,to)
		{
			float cut = kc*vel+vel*m->smoothers[CUTOFF1]->process();
			float q   = vel*m->smoothers[Q1]->process();
			float gain= vel*m->smoothers[GAIN1]->process();
			float slope= vel*m->smoothers[SLOPE1]->process();
	...
	{		
		...
		case PORT_SLOPE1:
			m->controls[SLOPE1] = (float*)data_location;
			break;
