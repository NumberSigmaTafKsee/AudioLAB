...
namespace Analog::Filters::Moog::MoogVCF
{
    ...
    private:
        ...
        // x[n-1]
        DSP previousInput = 0.0f;
        
        // y[n-1]
        DSP previousOutput = 0.0f;
    };
	...
	template<typename DSP>
    class MoogVCF : public GSSoundProcessor<DSP> {
    public:
        ...
        DSP delayed_filter_output = 0.0f;
    ...
    MoogVCF::MoogVCF(DSP sr) : GSSoundProcessor<DSP>()
    {
        sampleRate = sr;
    ...
    void MoogVCF::calculate_coefficients(DSP sampleRate, DSP frequencyHz, DSP resonance)
    {
        Fc = frequencyHz;
        R  = resonance;
    ...
    DSP MoogVCF::process(DSP in)
    {
		Undenormal denormal;
		
        DSP feedback_loop = in - ( (delayed_filter_output - (in * gComp)) * gRes * 4.0f );
    ...
    void MoogVCF::ProcessSIMD(size_t n, DSP * input, DSP * output)
    {
		Undenormal denormal;
		#pragma omp simd aligned(input,output)
		for(size_t i = 0; i < n; i++) 
		{
			DSP feedback_loop = input[i] - ( (delayed_filter_output - (input[i] * gComp)) * gRes * 4.0f );
