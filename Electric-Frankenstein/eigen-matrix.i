...
%inline %{
	...
	struct MatrixView
	{
		...
		MatrixView(Eigen::Matrix<T,X,Y,Z> &m, size_t r) : matrix(m), row(r) {
		
		}
		T __getitem__(size_t i) { return matrix(row,i); }
		void __setitem__(size_t i, const T & val) { matrix(row,i) = val; }
	};
...
{    
    ...
    public:
        ...
        %extend 
        {                      
            MatrixView<T,X,Y,Z> __getitem__(size_t i) { return MatrixView<T,X,Y,Z>(*$self,i); }            
