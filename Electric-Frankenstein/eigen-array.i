...
%inline %{
	...
	struct ArrayView
	{
		Eigen::Array<T,X,Y,Z> &array;
		size_t row;
		
		ArrayView(Eigen::Array<T,X,Y,Z> &m, size_t r) : array(m), row(r) {
		
		}
		
		T __getitem__(size_t i) { return array(row,i); }
		void __setitem__(size_t i, const T & val) { array(row,i) = val; }
	};
...
{    
    ...
    public:
        ...
        %extend {            
            ArrayView<T,_Rows,_Cols,_Z> __getitem__(size_t i) { return ArrayView<T,_Rows,_Cols,_Z>(*$self,i); }            
