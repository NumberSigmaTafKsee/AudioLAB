...
template<typename Sample> struct Thiran2Phaser {
  ...
  {
    phase += tick;
    if (phase > Sample(M_PI)) phase -= Sample(2*M_PI);

    auto lfo = lfoAmp * std::sin(phase + offset) - lfoMin;

    input = std::tanh(input + feedback * buffer);
    for (auto &ap : allpass) {
      ap.setup(lfo);
...
struct Thiran2Phaser16 {
  ...
  {
    phase += tick;
    if (phase > float(M_PI)) phase -= float(2*M_PI);
