    ...
    */
    ...
    public:
        ...
        */
        ...
        void ProcessBlock(size_t n, T* inputSample, T * outputSample);
	...
	template <class T>
    void LadderFilter<T>::ProcessSIMD(size_t n, T* inputSample, T* outputSample)
    {
		Undenormal denormal;
		#pragma omp simd aligned(inputSample,outputSample)
		for(size_t s = 0; s < n; s++)
		{
			for(int i = 0; i < mOversampleFactor; ++i) // repeat sample processing per oversampling factor
			{
				for(int filterIndex = 0; filterIndex < NUMBER_OF_FILTERS; ++filterIndex) // go through filter stages
				{
					...
					if(filterIndex == 0) // for the first filter
					{
						mvDiffVolt[filterIndex] = -mBiasControl * (tanh(mvVolt_1[filterIndex] * OVER_TWO_THERMAL_VOLT) + tanh((mGain * inputSample[s] + mResonance * mvVolt_1[NUMBER_OF_FILTERS-1]) * OVER_TWO_THERMAL_VOLT));
					}
					else // for the rest of the filters
					{
						mvDiffVolt[filterIndex] = mBiasControl * (tanh(mvVolt[filterIndex-1] * OVER_TWO_THERMAL_VOLT) - tanh(mvVolt_1[filterIndex] * OVER_TWO_THERMAL_VOLT));
			...
			// return the voltage output of the last filter
			outputSample[s] = mvVolt[NUMBER_OF_FILTERS-1];
		}
	}
}
