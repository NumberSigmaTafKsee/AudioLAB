...
namespace Analog::Calculus::RateLimiters
{
	...
	template<typename DSP>
    void SlewLimiter<DSP>::setParams(DspFloatType sampleRate, DspFloatType milliseconds, DspFloatType range) {
	...
	template<typename DSP>
    DspFloatType SlewLimiter<DSP>::next(DspFloatType sample, DspFloatType last) {
        if (sample > last) {
